...
**CHUNK 3 완료.** 발견: P0 1 / P1 2 / P2 0. **누적: P0 3 / P1 8 / P2 4.**

---

## CHUNK 4/7 — 시나리오 D (§42 기재불비): 동적 흐름 추적 + 도메인 품질 + 4축 점수

[누적 카운트: P0 3 / P1 8 / P2 4]

### 시나리오 D 입력 전제
```
본원:   청구항 6개 전부 거절
조문:   §42② 1호(명확성) + §42② 4호(뒷받침) 동시 지적  ← 구법 표기
현행법: §42④ 2호(명확하고 간결) + §42④ 1호(발명의 설명에 의해 뒷받침)
인용발명: 없음 (cited_refs:[])
```
...
파싱 prompt의 schema 예시가 `article:"§29②"`·`cited_refs:["인용문헌1"]`만 제시(CHUNK 1 발견 #3). §42 케이스에서 `cited_refs:[]`이어야 하는 패턴이 없으므로 LLM이 가짜 인용발명을 채울 위험. 단, 통지서 원문에 "인용문헌 없음"이 명시되어 있다면 LLM이 정상 처리할 가능성 있음.

**유형 판별 단계 입력 오염 가능성:** 파싱 결과에서 `article:"§42② 1호"` / `article:"§42② 4호"`가 구분된 두 항목으로 들어가면 유형 판별 LLM에 정확한 입력이 공급됨. 그러나 이 두 항목이 `rejection_reasons`에 올바르게 구분 저장되는지는 파싱 prompt가 보장하지 않음.

---

### D-흐름 2 — 유형 판별 (L1047-L1093)

유형 판별 prompt(L1082)의 schema hint:
```
B. description_deficiency — 기재불비(§42③④)
```
LLM이 통지서의 §42 조문을 인식하면 `primary_type:'description_deficiency'`를 반환할 가능성이 높음. 그러나 JSON 파싱 실패 시 `'inventive_step'` silent fallback (CHUNK 1 발견 #2). 기재불비 통지서에서 inventive_step으로 오분류되면 전체 파이프라인이 §29 법리로 흐름.

---

### D-흐름 3 — 분석 (L1188-L1191)

#### [P1] 분석 prompt deficiency_type enum이 §42 조문과 무매핑
**위치:** opinion.js L1191
**증거:**
```js
// L1191 — description_deficiency 분석 JSON schema
'"items":[{"claim_no":N,"deficiency_type":"unclear|inconsistent|unsupported",'
+'"examiner_comment":"심사관 지적 원문","spec_reference":"【0001】등 관련 단락",'
+'"suggested_correction":"구체적 수정 문언 제안"}]}'
```
`deficiency_type` 허용값: `unclear | inconsistent | unsupported`

**§42 조문과의 매핑 불명확성:**
| LLM이 선택할 값 | 심사관 지적 조문 | 실무 보정 방향 |
|----------------|----------------|--------------|
| `unclear` | §42④ 2호 (명확성·간결성) 추정 | 청구항 문언 명확화 — 권리범위 보존 가능 |
| `unsupported` | §42④ 1호 (뒷받침) 추정 | 청구항 문언을 명세서 기재 범위로 축소 — 권리범위 축소 |
| `inconsistent` | 어느 조문에도 명확히 매핑 불가 | — |

prompt가 조문-enum 매핑을 명시하지 않으므로 LLM이 자유 분류. 동일 사건에서 §42④ 1호 지적을 `unclear`로, §42④ 2호 지적을 `unsupported`로 반전 분류하면 보정 전략 역전.

**영향:** 명확성 흠결에 뒷받침 전략(권리범위 축소)을 적용하거나, 뒷받침 흠결에 명확성 전략(문언 명확화)을 적용 → 심사관이 "지적 미해소"로 거절 유지.

**개선 방향:**
```js
'"deficiency_type":"clarity_§42④2호|support_§42④1호|description_§42③|inconsistent",'
+'"article_cited":"§42④ 1호 또는 §42④ 2호 — 통지서 원문 조문 표기 그대로",'
```

---

### D-흐름 4 — 보정 (L1645-L1649)

#### [P0] spec_basis 환각 단락번호 — §47② 신규사항 실질 검증 없음 (시나리오 D 특화 분석)
**위치:** opinion.js L1649 (description_deficiency 보정 JSON schema) / 검증 코드 전체
**증거:**
```js
// L1649 — corrected_claims schema
'"corrected_claims":[{"claim_no":N,"original":"원문","corrected":"수정문",'
+'"spec_basis":["【0001】"],...}],'
```
`spec_basis`는 LLM이 수정 청구항의 명세서 근거로 반환하는 단락번호 배열이다.

**코드 전체에서 spec_basis 교차 검증하는 부분:**
```
grep 'spec_basis' opinion.js → L1630, L1643-L1644, L1649, L1657-L1659
```
모두 prompt 지시문 또는 JSON schema 정의. **spec_basis 배열의 단락번호가 실제 명세서 원문 텍스트에 존재하는지 코드 레벨에서 검사하는 곳은 없음.**

**§42 기재불비 대응에서 이 결함이 P0인 이유:**
- 진보성(§29) 대응은 인용발명과의 차이 논변이 핵심이므로 spec_basis는 보조 근거
- 기재불비(§42④ 1호) 대응은 "수정된 청구항 문언이 명세서 【XXXX】 단락에 직접 기재되어 있으므로 §47② 위반이 아님"이 **해소 논리의 전부**
- LLM이 존재하지 않는 `【0087】`을 spec_basis로 반환하면:
  1. 의견서 본문에 "본 수정 사항은 명세서 【0087】에 기재된 범위 내"라고 기술됨
  2. 심사관이 실제 명세서를 검토하면 【0087】에 해당 내용 없음을 즉시 발견
  3. 거절이유 해소 실패 → 거절결정으로 이어질 수 있음
- CHUNK 1 발견 #5는 startValidation ctx 미흡을 다뤘으나, 이 발견은 **출력 의견서 본문에 허위 단락 번호가 그대로 인쇄된다**는 더 치명적인 하류 결과를 다룸

**영향:** §42 기재불비 의견서의 핵심 논거(명세서 뒷받침 단락)가 환각 가능. KIPO 심사관이 의견서 제출 후 발견.

---

### D-흐름 5 — 의견서 (L1824-L1829)

#### [P1] §42 조문 표기 오류 — §42③④ vs 현행법 §42④ 1·2호
**위치:** opinion.js L12, L1082, L1824
**증거:**
```js
// L12 — TYPES 정의
description_deficiency: { ..., law:'§42③④' }

// L1082 — 유형 판별 prompt
'B. description_deficiency — 기재불비(§42③④)\n'

// L1824 — 의견서 prompt
'기재불비 위반(§42③④) 의견서를 작성해 주세요.'
```

**현행법 분석 (2007년 개정 이후):**
| 코드 표기 | 실제 조항 | 내용 |
|----------|---------|------|
| `§42③` | §42③ | 발명의 **설명** 작성 요건 (당업자가 실시 가능한 정도) |
| `§42④` | §42④ 1호 | 청구항이 발명의 설명에 의해 뒷받침될 것 |
| (없음) | §42④ 2호 | 청구항이 명확하고 간결하게 기재될 것 |

"§42③④" 표기의 문제:
1. §42③은 심사관이 청구항 거절에 인용하지 않는 조항 (발명의 설명 요건)
2. 청구항 기재불비의 정확한 조항은 §42④ 1호 / §42④ 2호
3. LLM에 `§42③④`를 주입하면 LLM이 §42③과 §42④를 동시 인용하여 의견서를 작성할 가능성 → 조항 표기 오류 의견서 생성

구법(2007년 이전) 조항인 `§42② 1호`, `§42② 4호`로 통지서가 발행된 경우(시나리오 D mock fixture의 관찰과 일치)에도, 시스템이 현행법 §42③④로 응답하면 조항 표기 불일치.

#### [P1] 기재불비 두 조문의 의견서 분리 논변 없음
**위치:** opinion.js L1828-L1829
**증거:**
```js
// L1828-1829
+'## 3. 구체적 의견내용\n'
...
```
"지적사항별" 처리를 지시하지만, §42④ 1호와 §42④ 2호가 동시에 지적된 경우 두 조문을 별도 소절로 분리하여 논변하도록 강제하지 않는다.

**KIPO 실무 요건:**
- §42④ 1호(뒷받침) 해소: 수정된 청구항이 명세서 기재 범위 내임을 단락 수준에서 증명
- §42④ 2호(명확성) 해소: 수정된 문언이 당업자에게 명확하게 이해됨을 논증
- 두 논변을 하나의 단락에 혼합하면 심사관이 "각 지적사항별 해소 논리 불명확"을 이유로 의견서를 불채택할 수 있음
...
| 축 | 점수 | 주요 근거 |
|----|------|---------|
| 자동 흐름 완전성 | 🔴 35/100 | §42 조문 분리 없음, silent fallback 가능(§42→inventive_step), spec_basis 환각 무검증 |
| 도메인 정확성 | 🔴 30/100 | deficiency_type enum 조문 무매핑, §42③④ 표기 오류, 두 조문 혼합 논변 위험 |
| 출력 즉시 제출 가능성 | 🔴 20/100 | 보정서 별도 없음(발견 #15 동일), §47② 환각 단락번호 의견서 포함 위험 |
| 법리 완전성 | 🔴 25/100 | spec_basis 교차 검증 없음, §47③(보정 범위) awareness prompt 수준, 두 조문 독립 논변 없음 |

**시나리오 D CHUNK 4 신규 발견:**
- 발견 #16 [P1]: deficiency_type enum §42 조문 무매핑
- 발견 #17 [P0]: spec_basis 환각 단락번호 — §47② 신규사항 실질 검증 없음 (D 특화)
...
**CHUNK 6 완료.** 발견: P0 0 / P1 1 / P2 3. **누적: P0 4 / P1 14 / P2 8.**

---

## CHUNK 7/7 — 종합 점수표 + P0/P1/P2 목록 + 변리사 결론

---

### 4축 × 3 시나리오 — 출력 품질 매트릭스 (100점 기준)

| 축 | 시나리오 P (§29② 진보성) | 시나리오 D (§42 기재불비) | 시나리오 M (혼합) | 행 평균 |
|---|---|---|---|---|
| **자동 흐름 완전성** | 🟡 60 | 🔴 35 | 🔴 15 | 37 |
| **도메인 정확성** | 🟠 50 | 🔴 30 | 🔴 10 | 30 |
| **출력 즉시 제출 가능성** | 🔴 30 | 🔴 20 | 🔴 5 | 18 |
| **법리 완전성** | 🟡 55 | 🔴 25 | 🔴 15 | 32 |
| **열 평균** | **49** | **28** | **11** | **29** |
...
| 시나리오 | 현재 상태 | KIPO 제출 가능성 |
|---------|----------|----------------|
| P (§29 진보성) | ⚠️ 조건부 사용 가능 | 사후적 고찰 논거 수동 보완 + 보정서 별도 작성 필수 |
| D (§42 기재불비) | ❌ 사용 불가 | §42 조문 오류·spec_basis 환각·두 조문 혼합 논변 등 다중 P0 |
| M (혼합) | ❌ 구조적 표현 불가 | 두 거절이유 중 하나 구조적 누락 — 다음 OA 재발생 확정 |
...
| # | 발견 | 위치 | 청크 |
|---|------|------|------|
| #1 | `secondary_rejection_type` DB 저장 후 파이프라인 전체에서 미소비 — 혼합 거절 구조적 표현 불가 | L1088 | 1 |
| #2 | `callForJSON` `_parse_failed` 무음 fallback → `rejection_type='inventive_step'` — 헤더·본문 모순 원인 | L1087-L1089 | 1 |
| #15 | `downloadDocx`에 보정서(補正書) 타입 분기 없음 — KIPO 제출 패키지 미완성 | L2065-L2096 | 3 |
| #17 | `corrected_claims.spec_basis` 환각 단락번호 — 명세서 원문 교차 검증 코드 0건. §47② 신규사항 실질 검증 없음 | L1649 전체 | 4 |

#### P1 — 고위험 (14건)

| # | 발견 | 위치 | 청크 |
|---|------|------|------|
| #3 | 파싱 schema 예시가 `§29②`만 — §42 사례 패턴 이탈, LLM 환각 유도 | L923, L928 | 1 |
| #4 | `description_deficiency` 분석 prompt `deficiency_type:"unclear\|inconsistent\|unsupported"` — §42④ 1호/2호 무매핑 | L1191 | 1 |
| #5 | `startValidation` ctx에 명세서 원문 미포함 — spec_basis 검증 환각 위험 (일반) | L1706-L1714 | 1 |
| #6 | Gate 1·2·3 실제 차단 조건 없음 — 전략 미선택·검증 fail·빈 의견서 상태에서도 통과 | L1507-L1537 | 2 |
| #8 | `parseOpinionSections` `##`/`###`만 인식, 필수 섹션 검증 없음 | L1870-L1915 | 2 |
| #11 | §32/§33/§36/§45 미인식 거절유형 → `inventive_step` 무음 fallback | L10-L14, L1087 | 2 |
| #13 | 독립항 1항만 보정 하드코딩 — 거절된 청구항 3·5가 독립항이면 미처리 | L1612 | 3 |
| #14 | 사후적 고찰(hindsight) 반박 논거 의견서 prompt에 누락 | L1820 | 3 |
| #16 | `deficiency_type` enum §42 조문 무매핑 — 명확성 vs 뒷받침 보정 전략 역전 위험 | L1191 | 4 |
| #18 | `Opinion.TYPES.description_deficiency.law='§42③④'` — §42③은 청구항 거절 조항 아님 | L12, L1082, L1824 | 4 |
| #19 | §42④ 1호·2호 두 조문 분리 의견서 섹션 없음 — 혼합 논변 생성 위험 | L1828-L1829 | 4 |
| #21 | `unchanged_claims` 필드 미소비 — 미거절 청구항 보존 코드 레벨 검증 없음 | L1644, L1657 | 5 |
| #22 | 청구항 번호 재정렬 LLM 의존 — 종속항 인용 오류 시 §42④ 2호 신규 거절이유 발생 가능 | L1654 | 5 |
| #23 | `Opinion.open` 프로젝트 전환 시 state 7개 필드 미초기화 — 이전 프로젝트 데이터 누출 | L460-L468, L2152-L2194 | 6 |

#### P2 — 중위험 (8건)

| # | 발견 | 위치 | 청크 |
|---|------|------|------|
| #7 | `Opinion.usage` 프로젝트 전환 시 미리셋 — API 호출 횟수 세션 누산 표시 | L742, L467 | 2 |
| #9 | `validateNoTemplateContamination` 경고만·비차단, SAFE_PHRASES 과다 면제 | L337-L381 | 2 |
| #10 | `sanitizeTemplate` 60자 styleExample에 사건 특유 내용 포함 가능 | L312-L315 | 2 |
| #12 | `setStatus` catch가 예외 삼킴 — DB 실패 시 메모리·DB 불일치 무음 발생 | L2151 | 2 |
| #20 | 유형 선택 UI에 `secondary_type` 경고 없음 — 사용자 혼합 거절 인식 수단 없음 | L1096-L1113 | 5 |
| #24 | opinion.js 파이프라인 레벨 AbortController 없음 — 이탈 시 LLM 호출 미취소, DB 오염 위험 | opinion.js 전체 | 6 |
| #25 | localStorage 템플릿 키 user_id 미포함 — 공용 브라우저 다계정 영업비밀 누출 | L143, L150, L199 | 6 |
| (+) | `Opinion.state.files` 프로젝트 전환 시 잔류 | L114, L460 | 6 |

---

### 최우선 수정 로드맵 (P0 → P1 상위 6건)

| 우선순위 | 발견 | 예상 수정 범위 | 효과 |
|---------|------|--------------|------|
| 1 | #2 `_parse_failed` 무음 fallback → 사용자 수동 선택 강제 | L1079-L1090, L10줄 | 헤더·본문 모순 제거 |
| 2 | #15 보정서 출력 추가 (`downloadDocx` amendment 타입) | L2030-L2096, ~50줄 | KIPO 제출 패키지 완성 |
| 3 | #17 spec_basis 교차 검증 — 명세서 원문 텍스트 포함 후 단락 존재 확인 | startValidation ctx 수정 | §47② 환각 차단 |
| 4 | #1 secondary_rejection_type prompt 주입 (단기: 경고 팝업, 중기: 복합 파이프라인) | determineType 후처리, 발견 #20 포함 | 혼합 거절 인식 |
| 5 | #23 `Opinion.open` state 초기화 블록 삽입 | L463 이전 9줄 추가 | 프로젝트 간 데이터 누출 차단 |
| 6 | #6 Gate 차단 조건 추가 (Gate 1: 전략 선택 필수, Gate 2: fail 건수 확인) | L1509-L1526, ~10줄 | 검증 fail 상태 의견서 생성 차단 |

---

### 변리사 결론 (3행)

**1. 현재 시스템은 §42 기재불비 및 혼합 거절 케이스에서 KIPO 제출 불가 수준이다.** 기재불비 의견서는 spec_basis 환각 단락번호·§42③④ 조문 표기 오류·두 조문 혼합 논변으로 심사관 설득력이 없고, 혼합 거절은 secondary_rejection_type이 소비되지 않아 두 거절이유 중 하나가 구조적으로 누락된 채 의견서가 작성된다.

**2. 진보성(§29) 케이스도 보정서 별도 출력 부재와 사후적 고찰 논거 누락으로 즉시 제출이 불가하며, 변리사가 수동 보완 없이 시스템 산출물을 그대로 KIPO에 제출하면 거절결정 위험이 있다.** 특히 `downloadDocx`가 의견서(.doc)만 생성하고 보정서(KIPO 별지 제13호 서식)를 생성하지 않는 것은 서비스의 "즉시 제출 지원" 가정과 근본적으로 모순된다.

**3. P0 4건(발견 #1·#2·#15·#17)을 우선 수정하면 기본 신뢰도를 확보할 수 있으며, P1 14건 중 #6(Gate 차단)·#13(독립항 범위)·#18(§42 조문)·#23(state 초기화)을 병행하면 시나리오 P의 실무 사용이 가능해진다.** 시나리오 M(혼합)은 PIPELINE 구조 변경이 필요한 중기 작업이므로 단기 우회로 유형 판별 시 secondary_type 경고 팝업 + 수동 2회 프로젝트 진행 안내를 제공할 것을 권장한다.

---

**검증 완료.** 총 발견: **P0 4 / P1 14 / P2 8 = 26건.** 코드 수정 0건 / Supabase 쓰기 0건 / 실제 LLM 호출 0건.
