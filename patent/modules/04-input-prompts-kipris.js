...
[C12] 【출력 직전 자기검증】 (a) 본문의 모든 (NN)이 REFTABLE에 있는가 (b) REFTABLE의 모든 장치부호가 본문에 사용되었는가 (c) 번호·명칭 중복이 없는가 (d) 40자+ 문단/문장이 어디서도 완전 중복되지 않는가 (e) 장치=~한다·방법=~하는 단계 극성이 뒤바뀌지 않았는가 (f) 소괄호가 부호 표기 외 용도로 쓰이지 않았는가 (g) ★ 동일한 구성 명칭에 서로 다른 부호가 두 개 이상 배정되지 않았는가(하나의 명칭=하나의 부호 — 같은 명칭이 REFTABLE·본문에서 서로 다른 (NN)으로 나타나면 하나로 통일) (h) ★ 모든 문단이 종결어미("~다.")로 끝나는가(문장이 중간에 끊긴 절단 문단이 없는가)${_mathOn?' (i) 모든 수학식의 좌·우변 기호가 "여기서" 절에 정의되었고 수학식 참조 방향(상기/다음의)이 실제 위치와 일치하는가':''}. 하나라도 어긋나면 그 부분만 고쳐 정합시킨 뒤 출력하라.
