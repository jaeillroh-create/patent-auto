...
${T}\n[장치 청구범위] ${outputs.step_06||''}\n[장치 도면 설계] ${outputs.step_07||''}${(outputs.step_15&&(outputTimestamps.step_15||0)>(outputTimestamps.step_08||0))?'\\n\\n[특허성 검토 결과 — 아래 지적사항을 상세설명에 반영하여 보완하라]\\n'+outputs.step_15.slice(0,2000):''}${getFullInvention({stripMeta:true,deviceOnly:true})}${styleRef}`;}
    ...
    case 'step_08c':{
      ...
      const _cList=_concepts.map((ct,fi)=>{const fn=_cFig[fi]||'?';const td=CONCEPT_DIAGRAM_TYPES[ct.type]||{label:ct.type};const refs=(ct.refMap||[]).map(r=>r.label?`${r.signNumber}(${r.label})`:`${r.signNumber}(${_conceptRefFallbackName(ct)})`).join(', ')||(ct.refNums||[]).join(', ')||'미정';return `도 ${fn}: ${td.label} (참조번호: ${refs})`;}).join('\n');
...
${T}\n[방법 청구항] ${outputs.step_10||''}\n[방법 도면] ${outputs.step_11||''}\n[장치 상세설명] ${(outputs.step_08||'').slice(0,3000)}${step15Ref}${getFullInvention({stripMeta:true})}${styleRef}`;}
    case 'unified_cohesion':{
      ...
      const lengthPreset={compact:'약 3,000~4,000자',standard:'약 5,000~7,000자',detailed:'8,000~10,000자',maximal:'약 22,000~25,000자',custom:'약 '+(customDetailChars*Math.max(_uDevFig.length,1))+'자'}[detailLevel]||'약 5,000~7,000자';
...
[C5] 【용어 통일 — 기재불비 방지】 동일 개념은 청구항 용어를 기준으로 REFTABLE·장치본문·방법본문 세 곳에서 문자 그대로 동일 명칭으로만 지칭하라(조사·띄어쓰기 외 표기 변형 금지). 하나의 참조번호=하나의 명칭.
...
[C9] 【수학식 금지】 【수학식 N】 블록·수식·"수학식 N에 따르면" 참조를 본문에 삽입하지 마라(별도 단계 소관). 알고리즘은 자연어로만 설명하라.
...
[C11] 【분량·앵커 뒷받침】
  - 장치 상세설명: ${lengthPreset} 이상을 하한 목표로 하라(상한 아님). 청구항의 모든 구성요소를 빠짐없이 포함하고 각 핵심 구성요소마다 변형 실시예("한편, 다른 실시예에서 상기 [용어]는 [대안]일 수 있다")를 기술하라.
...
[C12] 【출력 직전 자기검증】 (a) 본문의 모든 (NN)이 REFTABLE에 있는가 (b) REFTABLE의 모든 장치부호가 본문에 사용되었는가 (c) 번호·명칭 중복이 없는가 (d) 40자+ 문단/문장이 어디서도 완전 중복되지 않는가 (e) 장치=~한다·방법=~하는 단계 극성이 뒤바뀌지 않았는가 (f) 소괄호가 부호 표기 외 용도로 쓰이지 않았는가. 하나라도 어긋나면 그 부분만 고쳐 정합시킨 뒤 출력하라.
...
지금부터 위 출력 계약(C1~C12)을 지켜 <<<REFTABLE>>> 블록부터 순서대로 출력하라.`;
    }
    case 'step_13':{
