...
[C11] 【분량·앵커 뒷받침】
  - ★ 장치 상세설명은 도면 1개당 ${dlCfg.charPerFig}(공백 포함) 이상을 최소 목표로 한다(상한이 아니라 하한). 앵커 종속항 뒷받침·정량적 근거 기술을 위해서는 이를 초과해도 무방하다.
  - ★ 장치 상세설명 총 분량 ${dlCfg.total}(공백 포함) 이상을 목표로 하며, 목표에 현저히 미달하지 않도록 각 구성요소를 충분히 상세하게 기술하라(선택한 분량을 반드시 채울 것). 청구항의 모든 구성요소를 빠짐없이 포함하고 각 핵심 구성요소마다 변형 실시예("한편, 다른 실시예에서 상기 [용어]는 [대안]일 수 있다")를 기술하라.
  - ${dlCfg.extra}
...
[C12] 【출력 직전 자기검증】 (a) 본문의 모든 (NN)이 REFTABLE에 있는가 (b) REFTABLE의 모든 장치부호가 본문에 사용되었는가 (c) 번호·명칭 중복이 없는가 (d) 40자+ 문단/문장이 어디서도 완전 중복되지 않는가 (e) 장치=~한다·방법=~하는 단계 극성이 뒤바뀌지 않았는가 (f) 소괄호가 부호 표기 외 용도로 쓰이지 않았는가${_mathOn?' (g) 모든 수학식 변수가 "여기서" 절에 정의되었고 수학식 참조 방향(상기/다음의)이 실제 위치와 일치하는가':''}. 하나라도 어긋나면 그 부분만 고쳐 정합시킨 뒤 출력하라.

[C13] 【마무리 산출 — 동일 컨텍스트 흡수】 위 본문과 완전히 정합하는 세 블록을 이어서 출력하라(용어·부호는 REFTABLE 기준, 본문과 표기 동일).
