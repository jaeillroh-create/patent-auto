...
★★★ 공간배치 규칙 ★★★
1. "허브" = 이 도면 안에서 가장 많은 연결을 가진 노드 (반드시 1개 지정 — 도면 배치용 개념일 뿐이며, 상세설명 본문에 "최다 연결"·"가장 많은 연결" 같은 전역 최상급 서술을 만들지 마라: 도면마다 허브가 있으므로 전역 "최다"는 성립하지 않는다)
...
- ★ [대상] 규칙: 도 2 이후 세부 구성 도면의 [대상]은 항상 "본 발명의 일 실시예에 따른 ${getDeviceSubject()}(100)"이다(하위 구성부는 [대상]이 될 수 없다 — 종전 "직계 상위 구성부" 허용 문구가 "요청 조립부(110)는 …를 포함한다"류 포함 관계 모순을 유발했음). 하위 구성부를 [대상]·포함 주체·소유격 주어로 삼아 "A부(110)의 B부(150)"·"A부(110)에 구비되는 B부(150)"처럼 소속을 뒤바꾸는 표현을 금지한다(본문 "도 N을 참조하면" 단락에도 동일 규칙 적용 — 계열 서술은 "${getDeviceSubject()}(100)의 …계열은"으로).
...
${T}\n[방법 청구항] ${outputs.step_10||''}\n[방법 도면] ${outputs.step_11||''}\n[장치 상세설명] ${(outputs.step_08||'').slice(0,3000)}${step15Ref}${getFullInvention({stripMeta:true})}${styleRef}`;}
    case 'unified_cohesion':{
      ...
      //   공염불이고, 생성형 검사(용어·예시·중복 등)가 라운드마다 재추첨되어 결함 16건을 고치며 8건을 새로
      //   만드는 고정점 수렴 실패(순환 오류)의 α-다리였다. 재작성 모드(FIX_TARGETS 존재)에서만 현재 본문을
      ...
      if(_fixTargets||_reviewNotes){
        ...
        if(_b8){
          _baseInject='\n\n★★★ [기저 텍스트 — 최소 수정 원칙(배치20-2)] 이번 작성은 백지 재생성이 아니라 아래 [현재 본문]의 "최소 수정"이다.\n'
            +'- FIX_TARGETS·REVIEW_NOTES가 지적한 부분만 고쳐라.\n'
            +'- 지적과 무관한 문장은 표현을 바꾸지 말고 그대로 옮겨 적어라(재표현·재구성·어순 변경 금지 — 무관한 문장을 바꾸면 새 결함이 생긴다).\n'
            +'- 출력 형식(센티넬 블록 계약)은 그대로 지키되, 각 블록의 내용은 [현재 본문]을 기저로 한다.\n'
            +'[현재 본문 — 장치 상세설명]\n'+_b8
            +(_b12?('\n\n[현재 본문 — 방법 상세설명]\n'+_b12):'')
            +(_bFin?('\n\n[현재 본문 — 마무리 블록]\n'+_bFin):'');
        }
      ...
      const _refPlanDirective=_refPlanBlk?('\n\n★★★ [확정 부호표 — 코드가 청구항에서 확정함. 절대 기준] ★★★\n아래 [확정 부호표]의 명칭·번호 쌍만 도면부호로 사용하라. ⛔ 새 번호를 만들거나, 같은 구성에 다른 번호를 붙이거나, 표의 명칭과 다르게(예: 하드웨어 상용어로) 지칭하지 마라. <<<REFTABLE>>> 블록에는 아래 표를 그대로 옮겨 적고, 본문의 모든 (NN)은 아래 표의 번호만 사용한다. ★ 표의 모든 번호는 본문에 최소 1회 실제 사용하라(부호표에 등재만 되고 본문 미사용인 부호를 남기지 마라). ★ 포함 관계 서술 규칙: 구성 계층의 최상위는 (100) 하나다 — 표에 \'하위 구성\' 표기가 없는 구성부는 전부 (100)에 직접 포함되며, 하위 구성부를 다른 구성부의 포함 주체로 서술하지 마라(\'X부(110)는 Y부(150)를 포함한다\'·\'X부(110)에 구비되는 Y부(150)\' 금지 — 같은 부호가 두 부모를 가지면 도면을 그릴 수 없다). ★ \'최다 연결\'·\'가장 많은\' 등 전역 최상급의 허브 서술을 본문에 쓰지 마라(도면별 배치 중심은 \'도 N에서 중심에 배치된다\'로 도면-한정).\n[확정 부호표]\n'+_refPlanBlk+'\n'):'';
...
${designComponents}${_fixInject}${_reviewInject}${_baseInject}

지금부터 위 출력 계약(C1~C13)을 지켜 <<<REFTABLE>>> 블록부터 순서대로 출력하라.`;
    }
    case 'step_13':{
      // ★ [배치20-2] 역할 분리 — AI 검토는 "실질"(뒷받침의 실질·논리·특허성)만 담당한다.
      //   용어·부호·형식·중복 등 결정론 검사는 기계검증(validateSpecification/validateClaims) 전담이며,
      //   그 결함은 FIX_TARGETS 로 재작성에 직접 주입된다. 종전의 machineFindingsForReview 주입(기계결함을
      //   AI 가 재서술 → REVIEW_NOTES 와 FIX_TARGETS 로 같은 결함이 이중 주입)은 제거했다.
...
★ 역할 분담(중요): 용어 표기·도면 부호·청구항 형식·문단 중복·문장 절단 같은 형식·정합 결함은 기계 검증이 별도로 전담 검출한다. 너는 아래 항목의 "실질 판단"에만 집중하라 — 형식·정합 지적을 반복하면 그 지적은 폐기된다.
...
[3] 수학식 의미 정합 검토 (※ 변수 정의 존재·교차참조 번호·번호 순서는 기계검증 전담 — 검토 생략)
- 수학식이 청구항의 기술적 구성과 실제로 대응되는지(수식의 의미가 그 구성의 동작을 나타내는지) 확인
- 수식의 물리적·수학적 의미가 본문 서술과 모순되지 않는지 확인
...
[5-A] ★ 의미적 동의어 혼용 검토 (※ 동일 부호·문자열 수준의 표기 혼용은 기계검증 전담 — 검토 생략) ★
- 기계가 잡지 못하는 "의미적 동의어" 혼용만 지적하라: 부호가 병기되지 않은 채 같은 개념을 다른 말로 부르는 곳
- 예: "음량 조절 값"/"목표 음량"/"최종 음량"이 같은 개념을 가리키는 혼용
- 각 발견에 대해: [혼용 용어들] → [채택 기준 용어] / [통일 사유] 형식으로 제시

[5-B] ★ 파라미터 실질 검토 (※ 변수 정의 존재 여부는 기계검증 전담 — 검토 생략) ★
- 보정/학습 계수(alpha 등)의 값 범위, 초기값, 과도 누적 방지 조건이 명시되어 있는지 확인
- 핵심 파라미터에 대해 값범위/초기값/갱신방식/제한조건이 기술되어 있는지 확인
...
[8] 청구항 명확성 검토 (특허법 제42조 제4항 제2호) (※ 불확정 용어 목록·카테고리 혼입은 기계검증 전담 — 검토 생략)
- 청구항의 기술적 범위가 모호하거나 과도하게 광범위하지 않은지(실질 판단) 확인
- 구성요소 간 관계·동작 조건이 특정 가능한 수준으로 기재되어 있는지 확인
- 미흡한 경우: 해당 청구항 번호와 문제점을 지적하고 수정 방향을 제시하라

[9] 용어 실질 검토 (※ 청구항↔상세설명 명칭 문자열 일치·참조번호 대응은 기계검증 전담 — 검토 생략)
- 약어·전문용어가 상세설명에서 정의 없이 사용되지 않았는지 확인
- 같은 개념이 의미가 다른 용어로 서술되어 기술 내용이 달라 보이는 곳이 있는지 확인

[10] (기계검증 전담 — 검토 생략) 도면 부호 정합은 확정 부호표(refPlan)·결정론 정합(enforce)·기계검증이 전담한다. 이 항목은 "해당 없음"으로만 표기하라.

[11] (기계검증 전담 — 검토 생략) 청구항 형식(젭슨·인용관계·다중종속·형식적 종속항)은 기계검증(validateClaims)이 전담한다. 이 항목은 "해당 없음"으로만 표기하라.

[12] ★ 예시도/개념도 실질 검토 (※ 부호 기재 존재·명칭 일관·"도 N을 참조하면" 존재 여부는 기계검증 전담 — 검토 생략)
- [예시도/개념도 설계]가 제공된 경우에만 검토하라(없으면 "해당 없음"으로 표기).
- 각 예시도의 기술(記述)이 내용상 충분한지(형태·구성·의미가 실제로 설명되는지) 확인하라. 부실 시 지적하되, ⛔ 예시도 단락을 [상세설명](장치)에 추가하라고 제안하지 마라 — 장치 상세설명에 예시도를 끼워넣는 것은 금지이며, 예시도 보완은 별도 단계(예시도 상세설명) 소관이다.
...
// ═══ [배치21-1] AI 교열 프롬프트 — 검출의 LLM 전면 이관 ═══
// 정책(규칙 레이어 동결): 외부 검토가 새 결함 유형을 찾으면 결정론 규칙을 추가(두더지잡기)하지 않고
// 이 점검표에 일반화해 편입한다. 결정론 레이어는 이미 '삭제 불가능' 게이트(배치20-10)로 안전해졌으므로
// 유지·보수만 하고, 언어 결함(비문·탈락·소속·의미)은 LLM 검수가 전담한다.
// 수정 방식: 전문 재작성 금지 — LLM 은 "문장 단위 국소 패치"만 산출하고, 코드가 게이트(원문 일치·
...
★★★ 절대 원칙 ★★★
1. 전문 재작성 금지 — 결함 문장만 골라 최소 수정한다. before 는 본문의 문장을 한 글자도 바꾸지 말고 그대로 옮겨 적어라(코드가 문자열 일치로 적용하며, 불일치 패치는 폐기된다).
2. 참조부호 보존 — after 는 before 의 참조부호 (NN) 집합을 그대로 유지하라(추가·삭제·번호 변경 금지).
3. 청구범위·도면은 읽기 전용 컨텍스트다(명칭·구성 계층의 기준). 청구항 자체의 문제는 patches 가 아니라 reports 로 보고하라.
4. 기술 실질을 바꾸지 마라 — 문장 성분 복원·표기·정합만 고친다. 탈락 성분은 문맥·청구범위에서 근거를 찾아 복원하되, 근거를 특정할 수 없으면 patches 대신 reports 로 보고하라.
5. 숫자 사실관계(통계 수치·건수·조합 수)는 수정 금지 — 다만 문서가 제시한 정의로 계산 가능한 값(도메인 크기의 곱·비율·차분 등)을 검산하여 명시 수치와 어긋나면 reports 로 보고하라(예: 축별 값 수의 곱 vs 명시된 셀 수, 점유율 배제 후 잔여 비율).

[점검표 — 각 항목을 본문 전체에 전수 적용]
A. 문장 성분 탈락 비문: 목적어·주어·의존명사(시/경우/때)·연결어미가 빠져 비문이 된 문장. 예: "X부(110)가 생성한 Y부(120)로 전달되고"(목적어 탈락), "검증 미통과 대체 참조 산출부(170)의 결과로"('시' 탈락), "가용한 순위 산출부(150)는"('경우' 탈락), "n_데이터 저장부(130)에"(수식 기호 주어 훼손).
B. 소속·구성 계층 오류: "X(nnn)의 Y(mmm)"·"X는 Y를 포함한다" 서술이 [구성 계층]과 모순(Y 가 X 의 하위가 아님). 문서 전체의 포함 서술이 하나의 트리를 이루는지 검사하라 — 같은 부호가 두 부모를 가지면 모순이다(예: 도 2에서는 형제, 도 4에서는 하위). 도면 참조 단락·도면의 간단한 설명 포함.
C. 명칭·부호 정합: 동일 구성부가 청구범위·본문·부호의 설명·요약서 간 서로 다른 명칭으로 지칭(축약형 혼용 포함 — 청구범위 명칭 기준 통일), 문장에서 절취된 유령 명칭·플레이스홀더 잔재("장치 본체/시스템"류), 개념·비물리 대상에 대한 부호 부여, 청구항 구성요소의 부호 미부여, 동일 용어의 서로 다른 층위 이중 사용(예: 축 값의 "판단 불가" vs 판정 상태의 "판단 불가").
D. 의미 역전·내부 모순: 발명 취지와 반대로 읽히는 문장(예: "필요 최소한의 범위로만이 아닌"), 전역 최상급("최다"·"가장"·"유일")의 중복 선언(예: 세 구성부가 각자 최다 연결 허브라고 선언 — 도면 한정 표현으로 교정), 서로 독립인 변수를 일대일로 묶어 양방향 거짓 서술이 생기는 결합(예: 등급값과 완화 여부 — 완화 사실은 등급과 무관하게 별도 서술돼야 함), 규칙 2종 병존 시 우선 관계 미명시(예: 고정 해제 순서 vs 동적 선택), 문서가 스스로 제시한 기준과 어긋나는 서술(예: "정보 손실이 작은 순"이라며 도메인 크기 순서와 다른 나열).
E. 수식·파라미터 정합: 수식에 등장하는 모든 기호가 정의되는가, 본문이 참조하는 모든 임계값·하한·상한·계수에 값/범위/외부화가 기재되는가(미정의 파라미터), 아래첨자·연산자 잔재("argmax_는"·"Σ_는"류)와 오독 소지 기호(별표·하이픈 첨자), 범위·클램핑 서술 누락, 지시함수 등 기호 표기의 문서 내 통일, 수식-본문 서술 일치(정의역·등급 정수와 서술 등급명의 명시적 연결 포함).
F. 표기·관용구: 관용구 오기(예: "사상 및 필드" → "사상 및 범위"), 조사 오류, 명세서 문체(평서형 종결) 이탈.

[출력 — 설명·코드펜스 없이 순수 JSON 만. { 로 시작해 } 로 끝낸다]
{"patches":[{"section":"step_08","class":"A","before":"<본문 원문 그대로의 결함 문장>","after":"<최소 수정 문장>","reason":"<한 줄 사유>"}],
 "reports":[{"class":"R","where":"<위치>","note":"<수정 없이 보고할 사항 — 숫자 모순·청구범위 구조·근거 불특정 탈락 등>"}]}
- patches 는 심각한 것부터 최대 40건. 결함이 없으면 빈 배열이 정직한 출력이다.
- section 은 반드시 아래 [SECTION …] 블록의 id 를 그대로 쓴다.

[읽기 전용 — 청구범위(명칭·구성 계층의 기준)]
${claims}

[구성 계층 — 코드가 청구항에서 확정(포함 관계 서술의 기준)]
${(typeof _buildRefPlanBlock==='function'&&typeof refPlan!=='undefined'&&refPlan&&refPlan.length)?_buildRefPlanBlock(refPlan):'(부호표 미확정 — 청구범위 기준으로 판단)'}
