...
${T}\n[방법 청구항] ${outputs.step_10||''}\n[방법 도면] ${outputs.step_11||''}\n[장치 상세설명] ${(outputs.step_08||'').slice(0,3000)}${step15Ref}${getFullInvention({stripMeta:true})}${styleRef}`;}
    case 'unified_cohesion':{
      ...
      const _refPlanDirective=_refPlanBlk?('\n\n★★★ [확정 부호표 — 코드가 청구항에서 확정함. 절대 기준] ★★★\n아래 [확정 부호표]의 명칭·번호 쌍만 도면부호로 사용하라. ⛔ 새 번호를 만들거나, 같은 구성에 다른 번호를 붙이거나, 표의 명칭과 다르게(예: 하드웨어 상용어로) 지칭하지 마라. <<<REFTABLE>>> 블록에는 아래 표를 그대로 옮겨 적고, 본문의 모든 (NN)은 아래 표의 번호만 사용한다. ★ 표의 모든 번호는 본문에 최소 1회 실제 사용하라(부호표에 등재만 되고 본문 미사용인 부호를 남기지 마라).\n[확정 부호표]\n'+_refPlanBlk+'\n'):'';
...
5. 숫자 사실관계(통계 수치·건수·조합 수)는 수정 금지 — 검산 모순이 보이면 reports 로만 보고하라.
...
B. 소속·수식 관계 오류: "X(nnn)의 Y(mmm)" 서술이 청구범위의 구성 계층과 모순(Y 가 X 의 하위 구성이 아님). 도면 참조 단락과 도면의 간단한 설명 포함. 예: "데이터 저장부(110)의 요청 처리 계열은" → 서버(100) 소속이 맞음.
C. 명칭·부호 정합: 동일 구성부가 문서 내 서로 다른 명칭으로 지칭(청구범위 명칭 기준 통일), 문장에서 절취된 유령 명칭("직후 상기 임계값의 충족 여부"류), 개념·비물리 대상에 대한 부호 부여, 청구항 구성요소의 부호 미부여.
D. 의미 역전·내부 모순: 발명 취지와 반대로 읽히는 문장(예: "필요 최소한의 범위로만이 아닌"), 문서 내 상호 모순 서술(동일 역할의 중복 선언, 상태 정의 개수 불일치 등).
E. 수식 정합: 수식에 등장하는 모든 기호가 정의되는가, 아래첨자·연산자 잔재("argmax_는"·"Σ_는"류), 범위·클램핑 서술 누락, 지시함수 등 기호 표기의 문서 내 통일, 수식-본문 서술 일치.
