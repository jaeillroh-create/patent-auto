...
function computeDeviceLayout2D(nodes,edges,figNum){
  ...
  if(strategy==='TOPOLOGICAL'){
    ...
    // 위상 정렬에서 빠진 노드(고립) 추가
    allIds.forEach(id=>{if(!visited.has(id))topoOrder.push(id);});
    ...
    const hubId=allIds.reduce((best,id)=>{
      ...
      const ra=parseInt((nodes.find(nd=>nd.id===id)?.label.match(/\((\d+)\)/)||[])[1])||9999;
...
function runValidation(){const all=[outputs.step_06,outputs.step_10].filter(Boolean).join('\n');if(!all){App.showToast('검증할 청구항이 없어요','error');return;}const iss=validateClaims(all),el=document.getElementById('validationResults');if(!iss.length){el.innerHTML='<div class="issue-item issue-pass"><span class="tf">🎉</span>모든 검증 통과</div>';return;}el.innerHTML=iss.map(i=>`<div class="issue-item ${i.severity==='CRITICAL'?'issue-critical':'issue-high'}"><span class="tf">${i.severity==='CRITICAL'?'🔴':'🟠'}</span>${App.escapeHtml(i.message)}</div>`).join('');}
