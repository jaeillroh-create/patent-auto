...
test('★20-9a — 정형문 오기 원본 수정: "사상 및 필드" → "사상 및 범위"(3개 문서 공통 재발의 근원)', () => {
  ...
  // ★ [배치21-1 갱신] 검사 범위를 정형문 원문 컨텍스트("…로부터")로 한정 — AI 교열 점검표가 이 오기를
  //   "결함 예시"로 인용하는 것은 정당한 등장이므로 제외.
  assert.ok(!PATENT_SRC.includes('사상 및 필드로부터'), '★ 오기 잔존 0(정형문 STEP8_SUFFIX 가 원본이었음 — LLM 오기가 아님)');
});
