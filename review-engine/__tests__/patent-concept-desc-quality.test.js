...
test('B1 ★ step_08 예시도 블록 — 장치 구성(100~)에 의한 동작·구현(SW→HW)·실질 시나리오 지시', () => {
  const p = step08();
  assert.match(p, /\[장치 도면 설계\]의 어느 구성\(100~[^]*?에 의해 어떻게 동작·구현되는지/, '★ 장치 구성에 의한 동작·구현 지시');
  assert.match(p, /소프트웨어적 구성이 하드웨어\(장치 구성\)에 의해 구체적으로 동작·구현됨/, '★ SW→HW 구현 명확화(§42)');
  assert.match(p, /실질적·예시적 시나리오/, '★ 실질 시나리오');
  assert.match(p, /검색창\(31\)은 프로세서\(120\)가 실행하는 소프트웨어/, '★ 예시(31↔120 연결) 제시');
});
...
test('B1 소스 ★ 예시도 블록 동작·구현·시나리오 지시', () => {
  assert.match(PATENT_SRC, /예시도의 각 요소\(31~99\)가 \[장치 도면 설계\]의 어느 구성\(100~[^]*?동작·구현되는지/, '★ 소스 동작·구현 지시');
  assert.match(PATENT_SRC, /실질적·예시적 시나리오\(동작 흐름\)/, '★ 소스 실질 시나리오');
});

// ─────────── 회귀 ───────────

test('회귀 ★ #212 예시도 1급화(반드시 빠짐없이) + 장치 프레이밍 유지', () => {
  const p = step08();
  assert.match(p, /예시도\(도 N\)도 장치 도면과 ★동일한 수준으로★[^]*?반드시 빠짐없이★ 기술하라/, '#212 예시도 강제 유지');
  assert.match(p, /\[예시도 설계 — 참조번호 31~99/, '#212 예시도 설계 전문 주입 유지');
