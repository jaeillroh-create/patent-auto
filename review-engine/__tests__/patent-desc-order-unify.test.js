...
test('T3 소스 ★ [배치20-2 갱신] Step 13 [12] — 실질 검토 축소 후에도 장치 끼워넣기 제안 금지 유지', () => {
  // [배치20-2] [12]는 실질(내용 충분성) 검토로 축소 — "도 N을 참조하면" 존재검사는 기계 전담으로 이관.
  //   단, 장치 상세설명에 예시도 단락 끼워넣기 금지 가드는 실질 지적의 경계라 유지되어야 한다.
  assert.match(PATENT_SRC, /예시도 단락을 \[상세설명\]\(장치\)에 추가하라고 제안하지 마라/, '★ 장치 끼워넣기 제안 금지 유지');
  assert.match(PATENT_SRC, /예시도 보완은 별도 단계\(예시도 상세설명\) 소관/, '★ 예시도 상세설명 소관 라우팅 유지');
});
