...
test('회귀 ★ ?v= 릴리스 토큰(20260723-api-parse) + common.js(20260723a) 동기화', () => {
  const html = readFileSync(path.join(REPO, 'index.html'), 'utf8');
  assert.match(html, /patent\/patent\.js\?v=20260723-api-parse/, '★ patent.js 릴리스 토큰 갱신');
  assert.match(html, /shared\/common\.js\?v=20260723a/, '★ common.js 토큰 갱신(수정 반영)');
  assert.match(readFileSync(path.join(REPO, 'patent/patent.js'), 'utf8'), /version = '20260723-api-parse'/, '★ patent 로더 version 갱신');
});
