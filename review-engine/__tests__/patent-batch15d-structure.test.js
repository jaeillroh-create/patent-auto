...
test('★ D3 — ② 「전체」·④ 「본문만」 범위를 이름에 명시', () => {
  ...
  assert.match(HTML_SRC, /id="btnWfRewriteFixes"[^>]*>[\s\S]{0,80}진단 후 결함 반영해 다시 쓰기/, '★ ④ 진단+결함 반영 재작성 명시(배치18-5 단일 버튼)');
  assert.match(HTML_SRC, /④ 진단 후 결함 반영해 다시 쓰기 \(청구항·도면 유지\)/, '★ ④ 카드 타이틀(청구항·도면 유지)');
});
