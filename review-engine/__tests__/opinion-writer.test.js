...
test('호출 훅(T5 이연 해소): reviewState 있으면 renderDraft 가 ReviewUI.render 발화', () => {
  // window.ReviewUI 스텁 + reviewState 주입 → renderDraft 호출 시 발화 확인.
  let fired = null;
  win.ReviewUI = { render: (state, mount, opts) => { fired = { state, hasActor: !!(opts && opts.actor) }; } };
  Opinion.state.reviewState = { patchPlans: [{ id: 'plan_1', accepted: true, ops: [] }], issues: [], rounds: [], budget: {} };
  const mk = () => ({ innerHTML: '', appendChild() {} });
  const L = mk(), R = mk();
  Opinion.renderDraft(L, R, 'validated');
  ...
  Opinion.renderDraft(mk(), mk(), 'validated');
