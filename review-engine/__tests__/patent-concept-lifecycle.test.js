...
test('B2 ★ [배치20-2 갱신] Step 13 — [12] 예시도 실질 검토(형식·부호 검사는 기계 전담 스텁화)', () => {
  const p = step13();
  assert.match(p, /\[12\] ★ 예시도\/개념도 실질 검토/, '★ [12] 실질 검토 항목');
  assert.match(p, /기계검증 전담 — 검토 생략/, '★ 역할 분리 스텁 명시');
  // ★ [배치20-2] [10] 도면 부호 정합은 기계(refPlan·enforce·CHK-4/10) 전담 — AI 항목은 스텁으로만 존재
  assert.match(p, /\[10\] \(기계검증 전담 — 검토 생략\)/, '★ [10] 스텁화(부호 정합 AI 재위임 제거)');
});

test('B2 소스 ★ [배치20-2 갱신] [12] 실질 항목 + [5] 범위 8~12 유지', () => {
  assert.match(PATENT_SRC, /\[12\] ★ 예시도\/개념도 실질 검토/, '★ [12] 정의(실질만)');
  assert.match(PATENT_SRC, /위 1~4 및 8~12에서 발견된 문제/, '★ [5] 범위 8~12(스텁 포함 번호 체계 유지)');
