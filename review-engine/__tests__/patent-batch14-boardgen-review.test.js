...
// 5) ④ 문구
test('★ 5 — ④ 명세서 진단(AI) 카드에 특허성 포함 명시 + 문서 불변(배치15L)', () => {
  assert.match(HTML_SRC, /명세서 진단 \(AI\)/, '★ 카드 타이틀(진단으로 개명)');
  assert.match(HTML_SRC, /청구항 뒷받침·기재불비·특허성\(신규성·진보성\)<\/b>을 점검/, '★ 진단 항목 + 특허성');
  assert.match(HTML_SRC, /문서는 변경되지 않습니다/, '★ 진단은 문서 불변 명시');
});
