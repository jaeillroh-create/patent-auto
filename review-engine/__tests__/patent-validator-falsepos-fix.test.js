...
// ═══════════ 배치 2.2/2.4 — 부호의 설명 canonical 기준축 + 메커니즘 회귀 가드 ═══════════
// ★ 아래는 특정 코퍼스 카운트 복제가 아니라 "검출 메커니즘"을 고정하는 합성 픽스처다(실제 문서A=8/B=5는 기저진실).
//   FP 유발조건(비-부 명칭 '메모리', 공백 절단 '데이터 저장부', 동일접미 상이구성 '결과 산출부'/'신뢰도 산출부')을
//   심어 R1·R2·D1·D2·D3 회귀를 가드한다.

// [b]+[c] 기본: 부호 125가 canonical(자가보완부)과 다른 명칭(재정렬부) 배정 + 자가보완부 125·127 이중부호 = 2건
test('★ §2.2 canonical [b]+[c] 기본 — FP 유발조건 심어도 정확히 2건만', () => {
...
// [b] 단독: 부호 125가 canonical(자가보완부)과 다른 명칭(품질검증부) 배정 = 1건
test('★ §2.2 canonical [b] 단독 — 품질검증부/125 불일치만 1건', () => {
...
// 클린 + FP 유발조건 전부: 메모리(비-부)·데이터 저장부(공백절단)·결과/신뢰도 산출부(동일접미) → 0
test('★ §2.2 클린 — 메모리·공백절단·동일접미 상이구성 전부 오탐 0', () => {
...
// [D1] 완전일치 경로에도 ≥5자 통일 — generic 3자 '저장부' 상이 부호는 미발화(26P1036 저장부{131,133} 소거)
test('★ §2.4 D1 — generic 3자 "저장부" 완전일치 상이부호는 오탐 0(≥5자 게이트)', () => {
  const spec = '【발명을 실시하기 위한 구체적인 내용】\n저장부(131)는 그래프를 저장하고, 저장부(133)는 임시로 저장한다.';
  assert.equal(checks(spec, 'refnum_dupassign').length, 0, '★ "저장부"(3자) 비교키 <5 → 미발화');
});
// [D1] 진성 ≥5자 동일명칭은 여전히 발화(게이트가 진성까지 죽이지 않음)
test('★ §2.4 D1 회귀 — ≥5자 "실시간저장부" 동일명칭 상이부호는 발화 유지', () => {
  const spec = '【발명을 실시하기 위한 구체적인 내용】\n실시간저장부(131)는 저장하고, 실시간저장부(133)는 또한 저장한다.';
  assert.ok(checks(spec, 'refnum_dupassign').some(i => /실시간저장부/.test(i.message)), '★ 5자 이상은 발화');
});

// [D2] "자가 보완부" 확장 — '자가'가 조사('가')로 오인돼 절단되던 결함 수정 → 자가보완부(5자) 확보, {125,127} 발화
test('★ §2.4 D2 — "품질 계약 검증 및 자가 보완부" 확장이 자가보완부 확보(공통접미 ≥5) → 발화', () => {
  const spec = '【발명을 실시하기 위한 구체적인 내용】\n품질 계약 검증 및 자가 보완부(125)는 보완하고, 이후 품질 계약 검증 및 자가 보완부(127)가 재실행된다.\n\n【부호의 설명】\n품질계약검증및자가보완부 : 125';
  assert.ok(checks(spec, 'refnum_dupassign').some(i => /125.*127|127.*125/.test(i.detail)), '★ 자가보완부 {125,127} 발화(확장 복원)');
});

// [D3(i)] 도면요소 generic 시리즈(동일명칭 ≥3 연속번호) → dupassign 아닌 refnum_generic_series(MEDIUM) 1건 압축
test('★ §2.4 D3(i) — "데이터구조요소" 51~54 연속 시리즈는 generic_series MEDIUM 1건(dupassign 아님)', () => {
  const spec = '【발명을 실시하기 위한 구체적인 내용】\n데이터구조요소(51), 데이터구조요소(52), 데이터구조요소(53), 데이터구조요소(54)를 포함한다.\n\n【부호의 설명】\n데이터구조요소 : 51\n데이터구조요소 : 52\n데이터구조요소 : 53\n데이터구조요소 : 54';
  assert.equal(checks(spec, 'refnum_dupassign').length, 0, '★ 시리즈는 dupassign에서 제외');
  const ser = checks(spec, 'refnum_generic_series');
  assert.equal(ser.length, 1, '★ 시리즈당 1건 압축');
  assert.equal(ser[0].severity, 'MEDIUM', '★ MEDIUM');
  assert.ok(/51.*54|4개/.test(ser[0].message), '★ 51~54 범위 보고');
});
// [D3] 비연속 동일명칭(≥5자, 2개)은 여전히 dupassign HIGH(시리즈 아님)
test('★ §2.4 D3 회귀 — 비연속 자가보완부 {125,127}은 dupassign(시리즈 아님)', () => {
  const spec = '【발명을 실시하기 위한 구체적인 내용】\n자가보완부(125)는 보완하고, 자가보완부(127)가 재실행된다.';
  assert.equal(checks(spec, 'refnum_generic_series').length, 0, '★ 2개·간격2 → 시리즈 아님');
  assert.ok(checks(spec, 'refnum_dupassign').some(i => /자가보완부/.test(i.message)), '★ dupassign 유지');
});
...
test('★ 소스 — 배치2 신규검사 3종 + §6-7 파일명 스탬프', () => {
  ...
  assert.match(PATENT_SRC, /check:'refnum_generic_series'/, '★ §2.4 D3 generic 시리즈 검사');
  assert.match(PATENT_SRC, /완전일치·공통접미 모두 ≥5자 요구/, '★ §2.4 D1 비교키 ≥5');
  assert.match(PATENT_SRC, /단음절 주격 '이\/가' 제외/, '★ §2.4 D2 확장 정지어 수정');
