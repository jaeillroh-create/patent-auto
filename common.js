...
App.currentService = 'patent'; // 'patent' | 'trademark'
...
App.switchService = function(service) {
  ...
  // 패널 표시 전환
  document.getElementById('patent-panel').style.display = service === 'patent' ? 'block' : 'none';
  document.getElementById('trademark-panel').style.display = service === 'trademark' ? 'block' : 'none';
  
  // 해당 모듈 초기화
  if (service === 'trademark' && window.TM) {
    if (!TM.cache.loadedAt) {
      TM.init();
    } else {
      TM.renderDashboard();
    }
  } else if (service === 'patent' && window.Patent) {
    // 특허 모듈 새로고침 (필요시)
  }
...
// URL 해시 기반 초기 서비스 결정
App.initServiceFromHash = function() {
...
// ============================================================
// 2. 공통 프로젝트 로드 함수 (타입별 필터)
// ============================================================

App.loadProjects = async function(type = null) {
  try {
    let query = App.sb.from('projects').select('*');
    
    if (type === 'patent') {
      query = query.or('project_type.eq.patent,project_type.is.null');
    } else if (type) {
...
// ============================================================
// 3. KIPRIS API 설정 (환경변수에서 로드)
// ============================================================

App.kiprisApiKey = ''; // Supabase secrets 또는 환경변수에서 로드

App.setKiprisApiKey = function(key) {
  App.kiprisApiKey = key;
  if (window.TM) {
    TM.kiprisConfig.apiKey = key;
  }
};
...
// DOMContentLoaded 이벤트에서 호출될 추가 초기화
...
App.init = async function() {
  ...
  document.querySelectorAll('.service-tab').forEach(tab => {
    tab.addEventListener('click', () => {
      App.switchService(tab.dataset.service);
    });
  });
  
  // URL 해시 기반 초기 서비스
  App.initServiceFromHash();
  
  // 해시 변경 감지
  window.addEventListener('hashchange', () => {
    const hash = window.location.hash.slice(1);
    if (hash === 'patent' || hash === 'trademark') {
      App.switchService(hash);
    }
  });
};
...
window.onerror = function(msg, url, lineNo, columnNo, error) {
  ...
  if (error && error.name === 'ChunkLoadError') {
    App.showToast('파일 로드 실패. 페이지를 새로고침하세요.', 'error');
  }
