...
(function() {
  ...
  TM.loadKiprisKeyFromProfile = function() {
    ...
    // 2ìˆœìœ„: ê³„ì •ë³„ localStorage ìºì‹œ
    if (userId) {
      try {
        const cached = localStorage.getItem('tm_kipris_api_key_' + userId);
        if (cached) {
          TM.kiprisConfig.apiKey = cached;
          console.log('[TM] KIPRIS API í‚¤: localStorage ìºì‹œì—ì„œ ë¡œë“œë¨ (user: ' + userId.slice(0,8) + ')');
          return;
        }
      } catch(e) {}
    }
    
    // 3ìˆœìœ„: ë ˆê±°ì‹œ localStorage (ê³„ì • êµ¬ë¶„ ì—†ë˜ ê¸°ì¡´ í‚¤ â†’ ë§ˆì´ê·¸ë ˆì´ì…˜)
    try {
      const legacyKey = localStorage.getItem('tm_kipris_api_key');
      if (legacyKey && legacyKey !== DEFAULT_KEY) {
        TM.kiprisConfig.apiKey = legacyKey;
        console.log('[TM] KIPRIS API í‚¤: ë ˆê±°ì‹œ localStorageì—ì„œ ë§ˆì´ê·¸ë ˆì´ì…˜');
        // í”„ë¡œí•„ì— ì €ìž¥í•˜ê³  ë ˆê±°ì‹œ í‚¤ ì œê±° (ë¹„ë™ê¸°)
    ...
    console.log('[TM] KIPRIS API í‚¤: ê¸°ë³¸ê°’ ì‚¬ìš©');
  };
  
  // â˜… KIPRIS API í‚¤ë¥¼ í”„ë¡œí•„(Supabase)ì— ì €ìž¥
  TM.saveKiprisKeyToProfile = async function(kiprisKey) {
    ...
    try {
      ...
      // ë¡œì»¬ í”„ë¡œí•„ ìºì‹œ ì—…ë°ì´íŠ¸
      if (App.currentProfile) {
        App.currentProfile.api_key_encrypted = JSON.stringify(existing);
      }
      
      // userId-scoped localStorage ìºì‹œ
      try { localStorage.setItem('tm_kipris_api_key_' + userId, kiprisKey); } catch(e) {}
      
      console.log('[TM] KIPRIS API í‚¤: í”„ë¡œí•„ì— ì €ìž¥ ì™„ë£Œ');
    } catch(error) {
      console.error('[TM] KIPRIS API í‚¤ í”„ë¡œí•„ ì €ìž¥ ì‹¤íŒ¨:', error);
    }
  };
      ...
      <div class="tm-modal-overlay" onclick="TM.closeSettings()">
        <div class="tm-modal-content" onclick="event.stopPropagation()" style="max-width: 500px;">
          <div class="tm-modal-header">
            <h3 style="margin: 0; font-size: 18px; font-weight: 600;">âš™ï¸ ìƒí‘œ ì¶œì› ì„¤ì •</h3>
          ...
          <div class="tm-modal-body" style="padding: 24px;">
            ...
            <div class="tm-settings-section" style="margin-top: 20px;">
              ...
              <p style="margin: 0 0 12px 0; font-size: 12px; color: #6b7280; line-height: 1.5;">
                ë³€ê²½ì‚¬í•­ì´ ìžˆì„ ê²½ìš° ìžë™ìœ¼ë¡œ ì €ìž¥ë©ë‹ˆë‹¤.
              </p>
              <div style="font-size: 13px; color: #374151; line-height: 1.6;">
                <div>â€¢ ìž…ë ¥ í›„ 3ì´ˆ í›„ ìžë™ ì €ìž¥</div>
                <div>â€¢ 15ì´ˆë§ˆë‹¤ ì£¼ê¸°ì  ì €ìž¥</div>
              </div>
            </div>
            ...
            <div class="tm-settings-section" style="margin-top: 20px; padding: 12px; background: #f9fafb; border-radius: 8px;">
              ...
              <div style="font-size: 12px; color: #6b7280; line-height: 1.6;">
                <div>â€¢ KIPRIS API í‚¤: ${currentApiKey ? 'âœ… ì„¤ì •ë¨' : 'âŒ ë¯¸ì„¤ì •'}</div>
                <div>â€¢ ìžë™ ì €ìž¥: âœ… í™œì„±í™”ë¨</div>
              </div>
            </div>
          </div>
  ...
  TM.saveSettings = function() {
    ...
    if (newApiKey) {
      ...
      console.log('[TM] KIPRIS í‚¤ ì €ìž¥:', newApiKey.slice(0,8) + '... â†’ TM.kiprisConfigì— ë°˜ì˜ë¨');
      
      // â˜… í”„ë¡œí•„(Supabase)ì— ê³„ì •ë³„ ì €ìž¥
      TM.saveKiprisKeyToProfile(newApiKey);
      
      App.showToast('KIPRIS API í‚¤ê°€ ì €ìž¥ë˜ì—ˆìŠµë‹ˆë‹¤.', 'success');
    } else {
      TM.kiprisConfig.apiKey = DEFAULT_KEY;
      
      // â˜… í”„ë¡œí•„ì—ì„œë„ ì‚­ì œ
      TM.saveKiprisKeyToProfile('');
      
      // userId-scoped localStorage ìºì‹œë„ ì œê±°
      const userId = App.currentUser?.id;
      if (userId) {
        try { localStorage.removeItem('tm_kipris_api_key_' + userId); } catch(e) {}
      }
      
      App.showToast('ê¸°ë³¸ API í‚¤ë¡œ ë³µì›ë˜ì—ˆìŠµë‹ˆë‹¤.', 'info');
    }
